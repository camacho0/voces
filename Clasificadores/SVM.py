...
score_svm=cross_validate(model,X_train,Y_train,scoring=metri,cv=5,return_estimator=True)
print("Cross validation scores")
print("F1")
print(np.mean(score_svm["test_f1"]))
print(np.std(score_svm["test_f1"]))
print("Accuracy")
print(np.mean(score_svm["test_accuracy"]))
print(np.std(score_svm["test_accuracy"]))
print("Recall")
print(np.mean(score_svm["test_recall"]))
print(np.std(score_svm["test_recall"]))
#metricas
acc=[]
f1=[]
re=[]
mc=[]
for i in range(len(score_svm["estimator"])):
    Y_pred=score_svm["estimator"][i].predict(X_test)
